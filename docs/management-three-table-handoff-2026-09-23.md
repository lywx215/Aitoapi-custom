...
| 操作                             | Scope                             | 请求与要点                                                                                                                                                                                                                                                                                                                                                                                 |
| -------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `POST /accounts/import`          | `accounts:write`；验证另需 `accounts:test` | `{items:[{clientRef,credentials}],model?,verify?}`，202 `{data:{status:"queued",taskId}}`。1–100 项；`clientRef` 非空、至多 128 字符、同批唯一。`verify:false` 仅保存凭证并默认启用，验证由后续独立 test 执行；省略或 true 保持上传后独立验证。 |
...
| `PUT /accounts/{id}/credentials` | `accounts:write`；验证另需 `accounts:test` | 202 任务。旧格式为直接 storageState；新客户端宜用 `{credentials,expectedCredentialVersion,expectedStateVersion,verify:false}`。两个版本必须成对且为正整数；过期为 409 `VERSION_CONFLICT`。`verify:false` 原子替换并启用，不调用模型；省略或 true 保持先验证候选且保留原管理状态。 |
...
`verify:false` 成功的 import/replace 返回 `item.result={success:true,stage:"uploaded",credentialVersion,stateVersion}` 及独立的 `item.upload` 回执；`uploaded` 不表示模型通过。随后用 `/accounts/{id}/test` 指定目标模型；test 只读，不改变账号启用状态。
...
- `queued/running` 未完成；终态为 `succeeded/partial/failed/cancelled/interrupted`。逐项读取 `status` 与验证结果，`partial` 不是全体成功。新导入落库后默认启用，验证失败保留上传和当时启用状态；历史导入或后续业务停用仍可能为禁用。先用 item `accountId` 与账号读取核实实际状态，再决定后续 replace/验证/显式启用。
