...
本指南将帮助你在 [Zeabur](https://zeabur.com/) 上部署 `aitoapi-custom` 服务。
...
4. **配置应用**：填写以下参数：
   ...
   **镜像**：
   - **镜像名称**：`ghcr.io/mulan777/aitoapi-custom:latest`
   - **用户名**：不用填写
   - **密码**：不用填写
   ...
   **卷**：
   - 点击 **添加卷** 按钮。
   - 添加认证卷：**卷 ID** `auth`，**路径** `/app/configs/auth`。
   - 添加运行数据卷：**卷 ID** `data`，**路径** `/app/data`。

   `data` 卷用于保存模型探测结果、用量统计和账号路由状态。未挂载 `/app/data` 时，模型探测列表只能保留到当前容器生命周期结束。模型探测功能不需要新增环境变量。
